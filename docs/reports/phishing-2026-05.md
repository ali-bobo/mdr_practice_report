...
description: 社交工程釣魚 / 惡意程式投遞事件分析，端點防護自動攔截，零損害結案。
date: 2026-05
tags:
  - 釣魚
  - 惡意程式
...
!!! warning "聲明"
    本報告為個人學習與防禦知識分享之作品集。內容已匿名化處理，不含任何可識別特定組織或個人之資訊。威脅情報（IOC）引用自公開情報平台，僅供防禦研究參考，不構成對任何組織或個人的指控。
...
| 項目 | 內容 |
|---|---|
| **事件類型** | 社交工程釣魚 / 惡意程式投遞（malvertising） |
| **威脅結果** | 已由端點防護 (EDR) 偵測並自動隔離，未造成實質損害 |
| **偵測來源** | 端點防護 (EDR) |
| **事件月份** | 2026 年 5 月 |
...
使用者點選了一則惡意廣告，被導向外觀仿冒 ChatGPT 的釣魚網站。該網站以「流量過大、需下載桌面版應用程式才能繼續使用」為由，誘導使用者下載並執行安裝程式。

程式執行後，於系統植入偽裝成系統元件的惡意 DLL，並建立開機自動執行與常駐機制。端點防護於惡意檔案落地時即偵測並自動隔離，攻擊在造成實質危害前已被阻斷。
...
| **最終處置** | EDR 自動隔離 | 風險已解除（Risk Resolved） |
| **實際損害** | 無 | 攻擊於前段被攔截 |
...
![檔案屬性顯示名稱與描述身分不一致](../assets/images/phishing-2026-05/image6.png)
*圖六　檔案屬性顯示名稱與描述身分不一致*
...
![工作管理員「啟動」分頁中之 Profiler.exe](../assets/images/phishing-2026-05/image7.png)
*圖七　工作管理員「啟動」分頁中之 Profiler.exe*

### 4.5　偵測與隔離

端點防護於惡意 DLL 落地時即偵測並自動隔離 `Profiler.dll`，狀態標記為風險已解除。

![端點防護偵測並自動隔離惡意檔案](../assets/images/phishing-2026-05/image8.png)
*圖八　端點防護偵測並自動隔離惡意檔案*
...
!!! danger "事件應變"
    **若已下載或執行，立即通報資安 / IT 或 MDR 團隊，勿自行刪檔或重灌。**
    ...
    保留現場以利確認影響範圍。本案因循正常偵測與處理流程，得以零損害收場。
...
本案為一起透過惡意廣告散布、假冒 ChatGPT 之釣魚攻擊。使用者完成下載與執行，惡意程式成功落地並建立持續化機制，最終由端點防護於落地階段偵測並隔離，未造成實質損害。

本案凸顯三點：其一，攻擊起點為社交工程而非系統漏洞，使用者教育為第一道防線；其二，縱深防禦不可或缺，端點防護在前段防線失效後守住最後一關；其三，及早通報為控制事件規模之關鍵。
...
*本報告發布於個人作品集，內容已匿名化。如有問題請透過 About 頁面聯絡。*
